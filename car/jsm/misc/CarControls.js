...
var CarControls = ( function ( ) {
	...
	function CarControls( maxSpeed, acceleration, brakePower, turningRadius, keys ) {
		...
		this.brakePower = brakePower || 6.5;
	...
	CarControls.prototype = {
		...
		onKeyDown: function ( event ) {
			...
			switch ( event.keyCode ) {
				...
				case controlKeys.UP: controls.gearForward = true; break;

				case controlKeys.DOWN: controls.gearForward = false; break;
		...
		update: function ( delta ) {
			...
			var brakingDeceleration = 0.3;

			this.gearForward = controls.gearForward;
			...
			if ( ! ( controls.moveLeft || controls.moveRight ) ) {
				...
				if (Math.abs(wheelOrientation) < 0.05) wheelOrientation = 0;
			...
			//root.rotation.y =  (30/360)*(Math.PI*2)
